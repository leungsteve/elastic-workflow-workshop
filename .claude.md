...
The original `9.3.0-SNAPSHOT` (Nov 2025) build could not restore the 9.2.4 snapshot due to a `transport_version` metadata incompatibility. Upgrading the Instruqt VM to `9.4.0-SNAPSHOT` resolved this — the snapshot restores successfully.

**Verified doc counts after restore:**
- businesses: 100
- users: 75,992
- reviews: 148,385
- incidents: 0 (empty at snapshot time)
- notifications: 0 (empty at snapshot time)
