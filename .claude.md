# Claude Code Instructions for Negative Review Campaign Detection Workshop
...
|-----------|--------|
| Web App (port 8080) | ✅ |
| Bulk Attack Endpoint | ✅ |
| Detection Workflow | ✅ |
| Response Actions (protect/hold) | ✅ |
| Agent Builder Tools | ✅ |
| Agent Builder Agent | ✅ |
...
This project builds a workshop demonstrating Elastic's Workflows feature (headline for 9.3) using a negative review campaign detection scenario. The workshop is part of the "What's New in Elastic Search" series and will be delivered via Instruqt. The detection pattern applies to any review platform: Yelp, Amazon, Google, App stores, etc.
...
**Primary Specification:** See `docs/review-bomb-workshop-spec.md` for complete requirements.
...
├── docs/review-bomb-workshop-spec.md  # Full specification document
...
- **Specification:** See `docs/review-bomb-workshop-spec.md`
...
- [x] **Real Yelp data at scale** - COMPLETED: 14,338 businesses, 100K users, 1M+ reviews loaded from real Yelp Academic Dataset
...
1. **Created `admin/setup_agent_builder.py`** ✅
   ...
   - Creates 1 agent: `review_campaign_investigator` with all tools assigned
   - Supports `--delete` flag to clean up existing resources
   - Supports `--dry-run` to preview changes
   - Auto-derives KIBANA_URL from ELASTICSEARCH_URL if not set

2. **Updated Challenge 3 with Quick Setup Alternative** ✅
...
| **Script** | Run script → Task 6 | 3 min |
...
1. **Removed Dev Tools Alternatives from Challenge 3** ✅
...
2. **Added Agent Creation Task (Task 5)** ✅
   - New task: Create the Review Campaign Investigator Agent
   - Agent configuration:
...
1. **Fixed Incident Summary Tool (Task 2)** ✅
   ...
   - Fixed field names to match actual incidents index mapping:
     ...
     - `metrics.avg_stars` → `avg_rating`
     - `metrics.avg_trust` → `avg_trust_score`
     - `metrics.unique_attackers` → `unique_reviewers`

2. **Fixed Similar Reviews Tool (Task 4)** ✅
...
6. **Verified Detection Query Works** ✅
...
- Task 3: Analyze attack with Agent Builder (use Review Campaign Investigator agent)
...
| `app/models/business.py` | categories field validator for list→string |
...
1. **Added Automated Response Actions to Workflow** ✅
...
2. **Fixed Agent Builder Setup Script** ✅
...
3. **Full End-to-End Test Verified** ✅
...
```
[Cleanup] → [Attack] → [Detection] → [Response] → [Investigation]
    ↓           ↓           ↓            ↓             ↓
...
|-----------|--------|---------|
| Bulk Attack | ✅ | 15 reviews + attacker users created |
| Detection | ✅ | avg_trust=0.15, velocity=15/hr |
| Incident | ✅ | inc_c668d5d04e34, CRITICAL severity |
| Business Protected | ✅ | rating_protected=true |
| Reviews Held | ✅ | 15 reviews status=held |
| incident_summary | ✅ | Returns incident details |
| reviewer_analysis | ✅ | Shows CRITICAL/HIGH risk attackers |
| similar_reviews | ✅ | ELSER semantic search works |
...
## SESSION (2026-01-27) - GCS Snapshot/Restore for Instruqt Fast Setup

### Goal
Eliminate ELSER inference time during Instruqt startup by snapshotting pre-processed data to GCS.

### What Was Done

1. **Created `admin/create_snapshot.sh`** - One-time admin script for Elastic Cloud
   - Validates prerequisites (ES, ELSER, data files)
   - Creates indices, bulk loads data, waits for ELSER
   - Registers GCS repo and takes snapshot
   - Supports both API key and basic auth

2. **Created `instruqt/startup.sh`** - Kubernetes VM startup script
   - Restores snapshot from GCS on Instruqt kubernetes-vm
   - Uses `sa` GCS client with `readonly: true` and `verify=false`
   - 7-step flow: wait for pod → get credentials → wait for health → delete indices → register repo → restore → verify

3. **Modified `instruqt/challenges/01-getting-to-know-your-data/setup.sh`**
   - Simplified from bulk loading to snapshot verification only
   - Now verifies restore completed, waits for Kibana, enables Workflows, starts FastAPI

4. **Ingested data into Cloud clusters**
   - First attempt on 9.3.0 GA cluster, then re-ingested on 9.2.4 cluster
   - 100 businesses, 75,992 users, 148,385 reviews (with ELSER embeddings)
   - Used 500-doc batches for ELSER inference (5000-doc batches caused 504 timeouts)
   - Cleaned up ~2,500 duplicate reviews from retried batches

5. **Snapshot taken to GCS**
   - Bucket: `instruqt-workshop-snapshot-public`
   - Path: `elastic-whats-new-9.3.0/data/snapshot-v2`
   - Snapshot: `snapshot-v1` (ES 9.2.4, 5 indices, SUCCESS)

### Blocking Issue

**Snapshot cannot restore on Instruqt VM** due to ES version incompatibility:
- Instruqt runs `9.3.0-fe116a51-SNAPSHOT` (Nov 2025 pre-release build)
- This build cannot parse `transport_version` field in snapshot metadata from Cloud 9.2.4
- Also cannot restore from 9.3.0 GA (GA is considered "newer" than SNAPSHOT)
- Listing the snapshot works; only the restore operation fails

**Resolution pending:** Need either:
- A 9.3.0 GA image available in the Instruqt container registry
- ELSER available on the Instruqt VM to take snapshot from same build
- Fall back to optimized bulk loading without snapshot

### Files Changed

| File | Change |
|------|--------|
| `admin/create_snapshot.sh` | NEW - one-time snapshot creation |
| `admin/README.md` | Added snapshot/restore workflow docs |
| `instruqt/startup.sh` | NEW - kubernetes-vm snapshot restore |
| `instruqt/challenges/01-getting-to-know-your-data/setup.sh` | Simplified to snapshot verification |
| `docs/lessons-learned.md` | Added snapshot/restore section with all gotchas |
| `.claude.md` | This session update |

### Lessons Learned (see `docs/lessons-learned.md` for full details)

1. **Always verify exact ES build on target before creating snapshots**
2. **Test restore with a tiny snapshot before bulk ingestion**
3. **Use `_id` in bulk ops to avoid duplicate cleanup**
4. **Use fresh `base_path` when recreating snapshots** (stale metadata causes checksum errors)
5. **500-doc batches for ELSER bulk loading** (5000 causes 504 timeouts)
6. **Pre-check GCS permissions from both source and target clusters**
